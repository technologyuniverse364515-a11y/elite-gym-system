...
export function FloatingButtons() {
  ...
  return (
    <>
      ...
      {/* Sticky CTA Bar */}
      {showSticky && (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border glass-card px-4 py-3 animate-slide-up">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            <p className="hidden text-sm font-semibold text-foreground sm:block">{messages.stickyBarText}</p>
            <Link
              to={buttons.freeTrial.link}
              className="btn-primary-gym w-full rounded-lg px-6 py-2.5 text-center text-sm sm:w-auto"
            >
