            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-sm font-semibold uppercase tracking-wider transition-colors hover:text-primary ${
                  location.pathname === (link.to === "/" ? import.meta.env.BASE_URL : `${import.meta.env.BASE_URL}${link.to.startsWith("/") ? link.to.slice(1) : link.to}`) ? "text-primary" : "text-muted-foreground"
                }`}
              >
                {link.label}
              </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-lg font-semibold uppercase tracking-wider ${
                  location.pathname === (link.to === "/" ? import.meta.env.BASE_URL : `${import.meta.env.BASE_URL}${link.to.startsWith("/") ? link.to.slice(1) : link.to}`) ? "text-primary" : "text-foreground"
                }`}
              >
                {link.label}
              </Link>
